...
The repository contains the initial scope, repository design, selected technology stack, shared event contracts, the tenant/subject/role data model, deterministic two-tenant identities, resources and security configuration, tested tenant-context, actor-aware PostgreSQL resource, membership, configuration and lifecycle authorization, audited tenant suspension and teardown, Redis key-scoping and NATS subject/consumer controls, a machine-checked tenant CA hierarchy, a versioned client-certificate identity profile, a certificate issuance boundary with real two-issuer cryptographic verification, a tenant-scoped certificate inventory with append-only lifecycle records, guarded fresh-key renewal with atomic supersession, fail-closed application certificate-status validation, authorized permanent revocation, a transactional outbox that publishes source-authenticated certificate lifecycle events with causal identities to JetStream, a hardened NGINX client-mTLS gateway, certificate-authenticated profile and tenant-record read APIs backed by actor-aware forced RLS, a tested default-deny tenant role/action/resource-sensitivity matrix, bounded sensitive export and membership-review demonstration endpoints with server-generated operation IDs, and an explicit tenant-scoped PKI plus RBAC baseline mode that uses no adaptive trust inputs. Other application services and evaluation tools have not been implemented yet.
...
- [Tenant isolation phase verification](docs/architecture/tenant-isolation-verification.md)
...
- [Certificate renewal and key rotation](docs/architecture/certificate-renewal.md)
- [Certificate status validation](docs/architecture/certificate-status-validation.md)
- [Authorized certificate revocation](docs/architecture/certificate-revocation.md)
- [Signed certificate lifecycle events](docs/architecture/signed-certificate-events.md)
...
- [Role, action and resource-sensitivity matrix](docs/architecture/role-action-sensitivity-matrix.md)
- [Sensitive export and administration demonstration operations](docs/architecture/sensitive-demo-operations.md)
- [PKI plus RBAC baseline mode](docs/architecture/pki-rbac-baseline-mode.md)

Bootstrap the core services with `npm run infra:init`, `npm run pki:init`, `npm run policy:test`, `npm run infra:up`, `npm run infra:migrate` and `npm run demo:provision`, then run `npm run tenant-isolation:verify` for the Phase 2 gate, the individual `pki-definition`, certificate profile, issuance, inventory, renewal, status, revocation, event and recovery gates while diagnosing those boundaries, or `npm run certificate-lifecycle:verify` for the complete Phase 3 gate. Run `npm run mtls-gateway:verify` for the NGINX client-mTLS and protected application-identity boundary, `npm run gateway-spoofing:verify` for its forged-header and direct-bypass matrix, `npm run profile-record-api:verify` for the live profile and tenant-record API boundary, `npm run role-action-matrix:verify` for the exhaustive RBAC/sensitivity contract, `npm run sensitive-operations:verify` for bounded export/admin demonstrations, and `npm run pki-rbac-baseline:verify` for explicit Baseline B behavior without adaptive trust. Run `npm run foundation:check` for the complete foundation and `npm run foundation:clean` to verify a disposable first start without touching normal development data. Full commands, local ports, safe shutdown and deliberate reset steps are in the development guide.
