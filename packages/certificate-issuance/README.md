...
The private key stays with the subject. The signer receives the CSR plus an immutable instruction containing the profile-derived subject, tenant/subject URI SAN, client-auth usage and validity. After signing, the package parses the X.509 certificate and verifies its identity, SAN, key algorithm, client-auth usage, validity, serial, signature and immediate issuer certificate before returning public certificate metadata.
