...
Run `npm run runtime-isolation:verify` while the local stack is running. It proves that identical logical Redis cache and lock names create separate tenant keys, then connects with each tenant's NATS credentials to prove own-tenant delivery and broker rejection of cross-tenant subscriptions. Run `npm run cross-tenant:verify` for adversarial guessed-ID, forged-context, delimiter-collision and cross-tenant publish checks across all implemented boundaries. The verifications create only expiring Redis values and remove them on completion.
