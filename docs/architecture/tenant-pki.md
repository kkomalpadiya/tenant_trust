...
The static verifier is part of `npm run certificate-lifecycle:verify`, `npm run foundation:check` and the disposable `npm run foundation:clean` path. The Phase 3 gate adds real ephemeral Alpha/Beta issuers, exact-chain and cross-tenant negative tests, every implemented certificate state transition, signed lifecycle delivery and CA recovery while keeping all generated keys outside Git. Passing these gates does not claim that long-running tenant authorities or production HSM/KMS controls have been deployed. See [Complete certificate lifecycle verification](certificate-lifecycle-verification.md).
