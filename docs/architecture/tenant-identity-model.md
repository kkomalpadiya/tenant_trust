...
The durable model and local provisioning workflow do not authenticate requests, enforce suspension or enable row-level security. [Trusted tenant-context resolution](tenant-context-resolution.md) now defines how verified identity and this authoritative state produce one immutable tenant context while rejecting client-selected tenant switches. Database isolation and suspension enforcement remain later Phase 2 tasks.
